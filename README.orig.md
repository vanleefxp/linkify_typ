...
`{{name}}` is a Typst package that allows you to generate nicely-formatted links and URL strings to contents on different media platforms. It has two main sub-modules: the `{{name}}.url` sub-module which generates URL strings and the `{{name}}.display` submodule that allows you to insert a nicely-formatted clickable `link` element into your document.
...
* Creating links to contents.
    ...
    ```typst
    #import "@preview/{{name}}:{{version}}" as linkify: url as lu // abbreviation for `{{name}}.url`
    #import {{name}}.display: *
...
`{{name}}` 是一个用于生成并在文中插入互联网内容平台链接的 Typst 包，其具有两个主要的子模块，分别是用于生成字符串链接的 `{{name}}.url` 子模块，和用于生成可插入文档的格式化 `link` 元素的 `{{name}}.display` 子模块。
...
* 创建内容链接
    ...
    ```typst
    #import "@preview/{{name}}:{{version}}" as linkify: url as lu, // `{{name}}.url` 的简写
    #import {{name}}.display: *
